...
mb1 <- new("mixture_in_ball", domain = b1,
           ...
           weights = wts)
de <- density_at(mb1, sample_points(mb1, 1e5))
```
...
tmrs <- numeric(length(sigmas))
for (i in 1:length(sigmas)) {
  pars2 <- pars
  pars2@sigma <- sigmas[i] * diag(rep(1, d))
  mrs[i] <- new("simulation_summary_results", pars2, 10L)@misc_rate
  tmrs[i] <- theoretical_misc_rate(pars2, de)
}
plot(-log(sigmas), mrs, type = "o", xlab = "-log sigma", ylab = "misc. rate")
lines(-log(sigmas), tmrs, col = "red")
...
```{r, fig.width=6, fig.height=5}
ks <- 10L * as.integer((1:20)^2)
mrs <- numeric(length(ks))
tmrs <- numeric(length(ks))
