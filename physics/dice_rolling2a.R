...
simulate_dice <- function(init_pos = c(0.99, 3)) {
  ...
  #dice_pos <- c(0.99, 3)
  dice_pos <- init_pos
  dice_angle <- pi/4 + 0.01
  
  ## initial velocity of dice
  dice_v <- c(1, 0) # 2, 2.01, 2.1
  dice_omega <- 0
...
#    print(params$tt - oldparams$tt)
  ...
  do.call(get_vvs, params)
  c(a = params$dice_angle, 
    x= params$dice_pos[[1]],
    y = params$dice_pos[[2]], t = params$tt)#, params$dice_v, params$dice_omega)
}
...
table(lab[des[, 2] > 2])
res3 <- cbind(des, lab = lab, res2)


draw_cell <- function(i) {
  draw_dice(xl = c(-3, 3), yl = c(0, 5), dice_pos = res3[i, c("x", "y")], dice_angle = res3[i, "a"])
  title(res3[i, "lab"])
  print(res3[i, ])
}
...
#saveRDS(res3, file = "physics/dr2a.rds")

# simulate_dice(c(0.98, 1))
# simulate_dice(c(0.99, 1))
# 
# 
# simulate_dice(c(0.98, 1))
# simulate_dice(c(0.99, 1))
