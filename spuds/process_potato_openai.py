...
for ii in potato_is:
    ...
    for jj in rep_is:
        ...
        txt = np.loadtxt('potato_openai/' + fn, dtype = str, delimiter = '@')
        if txt.size > 2:
            try:
    ...
    if len(vs) > 1:
        ...
        potato_final_is.append(ii)
        potato_vecs.append(vv)

potato_dishes = np.loadtxt('wikipedia_dish_list.txt', dtype=str, delimiter = ':')

dishes_final = potato_dishes[potato_final_is]
np.savetxt("potato_open_ai_dishes.txt", dishes_final, fmt = "%s")
