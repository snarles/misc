...
cppFunction(
'NumericVector aggr(NumericVector x, NumericVector y, int k) {
  int n = x.size();
  NumericVector out(k);
...
aggr(c(2.0, -2.1, 3.1, 4.2), c(2, 2, 1, 0), 3)

cppFunction(
'int aggrInd(NumericVector x, NumericVector y, int k) {
  int n = x.size();
...
aggrInd(c(2.0, 3.0, 3.1, 4.2), c(2, 2, 1, 0), 3)
...
proc.time()
Bclusters2 <- greedo(B, Bclusters, 1)
proc.time()

Bclusters2 <- greedoC(B, Bclusters - 1, max(Bclusters))
