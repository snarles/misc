...
def test_defeated_by_and_defeats_are_disjoint():
    beaten_by = set(defeated_by("BFN").tolist())
    beats = set(defeats("BFN").tolist())
    assert not beaten_by & beats
    assert codon_index("BFN") not in beaten_by | beats  # draws with itself
...
@needs_matrix
def test_best_partner_for_bfn_is_bpn():
    """Reported in report.md; BPN also minimises the codons beating both."""
    matrix = load_beat_matrix()
    coverage = coverage_counts(defeated_by("BFN"), matrix)
    assert codon_text(int(coverage.argmax())) == "BPN"
    assert int(coverage.max()) == 12302
...
def test_partner_coverage_is_not_permutation_invariant():
    ...
    scores = {
        ...
        for variant in ("BPN", "BNP", "NBP", "NPB", "PBN", "PNB")
    }
