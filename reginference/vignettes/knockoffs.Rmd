...
Confirm that FDR works for fitted data
```{r, fig.show='hold'}
n <- dim(x)[1]
x <- scale(as.matrix(x))
res_lin <- lm(y ~ x)
...
x_false <- noised_projections(x, sigma = 0.1, q = 20)
x_aug <- cbind(x, x_false)
summary(lm(res_lin$fitted.values ~ x_aug))
y_lin <- res_lin$fitted.values + rnorm(n)
res_k <- knockoff.filter(x_aug, y_lin)
res_k$selected
ko <- scale(res_k$knockoff)
dim(x_aug)
image(rbind(t(ko) %*% x_aug, x_aug %>% { t(.) %*% .}))
diag(t(ko) %*% x_aug)
diag(x_aug %>% { t(.) %*% .})
plot(t(ko) %*% x_aug, x_aug %>% { t(.) %*% .})


plot3d(cbind(x, res_lin$fitted.values))
res <- knockoff_experiment(x, y_lin, fdr, 0.1, 10, 20)
ave_fdr <- apply(res, 2, mean)
plot(fdr, ave_fdr, ylim = c(0, 1))
abline(0, 1)
title("Linear UN")
...
Test if the culprit is nonlinearity: generate a new set
