...
object MovieLensALS {
  ...
  def main(args: Array[String]) {
    ...
    val ratings = sc.textFile(new File(movieLensHomeDir, "ratings.dat").toString, 50).map { line =>
      val fields = line.split("::")
      // format: (timestamp % 10, Rating(userId, movieId, rating))
      (fields(3).toLong % 10, Rating(fields(0).toInt, fields(1).toInt, fields(2).toDouble))
    }

    val movies_RDD = sc.textFile(new File(movieLensHomeDir, "movies.dat").toString, 50).map { line =>
      val fields = line.split("::")
